...
import { Plus, Edit2, Trash2, X, Link as LinkIcon, Loader2, Search, ExternalLink, Calendar, Upload } from 'lucide-react';
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 glass rounded-xl overflow-hidden border-orange-500/20 flex-shrink-0">
                      {formData.image.url ? <img src={formData.image.url} className="w-full h-full object-cover" /> : <Upload className="m-auto text-slate-400 mt-3" size={20} />}
                    </div>
                    <label className="flex-grow cursor-pointer glass hover:bg-slate-100 dark:hover:bg-slate-800 border-dashed border-white/40 border-2 rounded-xl h-12 flex items-center justify-center transition-all">
                      <input type="file" onChange={handleUpload} className="hidden" />
                      {uploading ? <Loader2 className="animate-spin text-accent-orange" size={16} /> : <div className="flex items-center gap-2"><Upload size={16} className="text-slate-400" /><span className="text-[10px] text-slate-500">Upload</span></div>}
                    </label>
                  </div>
                </div>
              </div>
